 ...
 *
 */
public class ExposureTestChris {
	...
	public void testExpectedPositiveExposure() throws CalculationException {
		...
		int seed = 123;
		LIBORModelMonteCarloSimulationInterface lmm = createLIBORMarketModel(Measure.SPOT, 10000, 5, 0.1, seed);

		// Print a single exposure path and the expected positive exposure
		System.out.println("obsDate \t exposureOnPath \t expectedPositiveExposure");
		for(double observationDate : lmm.getTimeDiscretization()) {
			...
			System.out.println(formatterGerman.format(observationDate) + "\t" + formatterGerman.format(exposureOnPath) + " \t " + formatterGerman.format(expectedPositiveExposure));
	...
	public static LIBORModelMonteCarloSimulationInterface createLIBORMarketModel(
			Measure measure, int numberOfPaths, int numberOfFactors, double correlationDecayParam, int seed) throws CalculationException {
