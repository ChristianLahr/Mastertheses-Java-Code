 ...
 public class ProductRuleTest
{
		...
		private RandomVariableInterface[] integralProcess=null;
	...
	private void generateProcess() {
        ...
        integralProcess=new RandomVariable[getNumberOfTimeSteps()+1];
		...
		integralProcess[0]=  new RandomVariable(0.0,  initialValue*initialValue); 
		...
		for(int timeIndex = 1; timeIndex < getNumberOfTimeSteps()+1; timeIndex++)
		{
			double[][] newGBMRealization = new double[2][numberOfPaths];   //reset the 
			 double[] newIntegralProcessRealization=new double[numberOfPaths];
				...
				for (int componentIndex = 1; componentIndex < numberOfPaths; componentIndex++ )
				{
					 ...
					 double previousIntegralProcessValue=integralProcess[timeIndex-1].get(componentIndex);	
					...
					newIntegralProcessRealization[componentIndex]=
							  + previousIntegralProcessValue+
			...
			integralProcess[timeIndex]=new RandomVariable(times.getTime(timeIndex), newIntegralProcessRealization);
		}
	}
...
public RandomVariableInterface getIntegralProcessAtSomeInstant(int timeIndex){
	if (geometricBivariateProcess== null){
		generateProcess();
	}
	return integralProcess[timeIndex];
}
...
//	}
   ...
   ProductRuleTest eulerSchemeCorrelatedProcess=new  ProductRuleTest(numberOfPaths, numberOfTimeSteps,  deltaT,  initialValue, volatility, drift,
			 correlation);
	...
	// System.out.println(eulerSchemeCorrelatedProcess.numberOfPaths);
	 ...
	 for(int timeIndex=0; timeIndex<eulerSchemeCorrelatedProcess.getNumberOfTimeSteps() ; timeIndex++){
		System.out.println(  //formatting in a table the data gathered for the given time instant (no need of the full power of RVs arrays here)
				...
				formatterSci4.format(eulerSchemeCorrelatedProcess.getIntegralProcessAtSomeInstant(timeIndex).get(testPathIndex))
						+ "\t" +   "        " +
